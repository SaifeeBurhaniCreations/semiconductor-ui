...
import { 
    ...
    Wifi, WifiOff, FileText, Crosshair, Navigation, Bell
} from 'lucide-react';
import { 
                            ...
                            <div className="h-full flex flex-col space-y-6">
                                ...
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex space-x-3">
                                        <button className="px-4 py-2 bg-quantum-900 hover:bg-quantum-800 border border-quantum-700 rounded text-xs font-bold text-slate-300 flex items-center transition-colors">
                                            <RotateCcw className="w-3 h-3 mr-2" /> Restart
                                        </button>
                                        <button className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded text-xs font-bold flex items-center shadow-glow-cyan transition-colors">
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="bg-quantum-900 border border-quantum-700 rounded-lg p-5 flex flex-col">
                                        ...
                                        <button className="w-full mt-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold rounded shadow-lg transition-colors">
                ...
                <div className="p-4 h-full overflow-y-auto custom-scrollbar">
                    <div className="space-y-6">
                        ...
                        <div className="bg-quantum-900 border border-quantum-700 rounded-lg p-5">
                            ...
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={MOCK_CLUSTER_DATA} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                        <CartesianGrid strokeDasharray="3 3" stroke="#1c2633" vertical={false} />
                                        <XAxis dataKey="name" stroke="#475569" fontSize={10} />
                                        <YAxis stroke="#475569" fontSize={10} />
                                        <Tooltip 
                                            cursor={{fill: '#1e293b'}}
                                            contentStyle={{ backgroundColor: '#0a0f16', borderColor: '#2b3a4a', color: '#e2e8f0' }} 
                                        />
                                        <Bar dataKey="load" name="Load %" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
                                        <Bar dataKey="temp" name="Temp (C)" fill="#ef4444" radius={[4, 4, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <EnvCard 
                                ...
                                status="warning" 
                                trend="spike" 
                                desc="ISO Class 3 Limit Approaching"
                            />
                        </div>
                        ...
                        <div className="bg-quantum-900 border border-quantum-700 rounded-lg p-5">
                            ...
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={MOCK_POWER_DATA} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                        <CartesianGrid strokeDasharray="3 3" stroke="#1c2633" vertical={false} />
                                        <XAxis dataKey="time" stroke="#475569" fontSize={10} interval={3} />
                                        <YAxis stroke="#475569" fontSize={10} />
                                        <Tooltip 
                                            contentStyle={{ backgroundColor: '#0a0f16', borderColor: '#2b3a4a', color: '#e2e8f0' }} 
                                        />
                                        <Line type="monotone" dataKey="usage" stroke="#eab308" strokeWidth={2} dot={false} name="Grid Usage (kW)" />
                                        <Line type="step" dataKey="solar" stroke="#22d3ee" strokeWidth={2} dot={false} name="Solar Offset (kW)" />
                                    </LineChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
...
const StatCard = ({ icon, label, value, sub, color }: any) => (
    <div className="bg-quantum-900 border border-quantum-700 p-4 rounded-lg flex items-center space-x-4">
        <div className={`p-3 bg-quantum-950 border border-quantum-800 rounded-full ${color}`}>
            {icon}
        </div>
        <div>
            <div className="text-xs text-slate-500 uppercase">{label}</div>
            <div className="text-xl font-mono font-bold text-slate-200">{value}</div>
...
const TelemetryCard = ({ icon, label, value, unit, color }: any) => (
    <div className="bg-quantum-900 border border-quantum-700 p-4 rounded-lg flex items-center justify-between">
        <div className="flex items-center space-x-3">
            <div className={`p-2 bg-quantum-950 rounded ${color}`}>{icon}</div>
            <div>
                <div className="text-xs text-slate-500 uppercase">{label}</div>
                <div className="text-lg font-bold text-slate-200">{value}</div>
            </div>
        </div>
        <div className="text-[10px] text-slate-500 font-mono text-right">{unit}</div>
    </div>
);

const FilterBadge = ({ label, count, color }: any) => (
    <button className={`flex items-center space-x-2 px-2 py-1 rounded text-[10px] font-bold border transition-colors ${
        color === 'red' ? 'bg-red-900/20 text-red-400 border-red-500/20 hover:bg-red-900/30' : 
        color === 'orange' ? 'bg-orange-900/20 text-orange-400 border-orange-500/20 hover:bg-orange-900/30' :
...
const MiniMetric = ({ label, value, warning }: any) => (
    <div className={`flex justify-between items-center text-[10px] px-2 py-1 rounded border ${warning ? 'bg-orange-900/20 border-orange-500/30' : 'bg-quantum-950 border-quantum-800'}`}>
...
const ControlButton = ({ icon, label, desc, active }: any) => (
    <button className={`flex items-start p-3 rounded-lg border transition-all text-left ${
        active 
        ? 'bg-cyan-900/20 border-cyan-500/50 text-cyan-400' 
        : 'bg-quantum-950 border-quantum-800 hover:border-quantum-600 text-slate-300 hover:text-slate-100'
    }`}>
...
const EnvCard = ({ icon, label, value, status, desc }: any) => (
    <div className={`p-4 rounded-lg border ${status === 'warning' ? 'bg-orange-900/10 border-orange-500/30' : 'bg-quantum-900 border-quantum-700'}`}>
        <div className="flex justify-between items-start mb-2">
            <div className={`p-2 rounded bg-quantum-950 ${status === 'warning' ? 'text-orange-400' : 'text-cyan-400'}`}>{icon}</div>
            {status === 'warning' && <AlertTriangle className="w-4 h-4 text-orange-500 animate-pulse" />}
        </div>
        <div className="text-2xl font-mono font-bold text-slate-200">{value}</div>
        <div className="text-xs text-slate-500 uppercase mt-1">{label}</div>
        {desc && <div className="text-[10px] text-orange-400 mt-2">{desc}</div>}
    </div>
);